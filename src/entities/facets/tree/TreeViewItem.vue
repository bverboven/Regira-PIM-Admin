    ...
    <div class="container-fluid m-0 p-0">
        <div class="row g-0 hover-line">
            ...
            <div class="col text-truncate">
                <span class="d-inline-block p-1" :class="{ 'fw-bold': isSelected }">
                    <Icon :name="isFacet ? FacetEntity.name : FacetGroupEntity.name" class="me-1 opacity-50" />
                    {{ nodeItem?.title }}
                </span>
            </div>
        </div>
        <TreeView v-show="node.value.isExpanded" :nodes="sortedChildren" :selected="selected" @toggle-node="$emit('toggle-node', $event)" class="tree-indent" />
    </div>
</template>
...
import { Entity as FacetEntity, FormModalButton as FacetModalButton, useEntityStore as useFacetStore } from "../../facets"
import { Entity as FacetGroupEntity, FormModalButton as FacetGroupModalButton, useEntityStore as useFacetGroupStore } from "../../facet-groups"
import type Facet from "../../facets/data/Entity"
...
const isFacet = computed(() => props.node.value.type === FacetEntity.name)
